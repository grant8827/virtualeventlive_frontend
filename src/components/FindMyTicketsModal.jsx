...
export default function FindMyTicketsModal({ onClose }) {
  ...
  const [visibleCode, setVisibleCode] = useState(null) // ticket id whose ticket card is expanded
                  ...
                  <div key={t.id} className="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
                    ...
                    <div className="p-4 flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        >
                          {visibleCode === t.id ? 'Hide Ticket' : 'View Ticket'}
                        </button>
                      </div>
                    </div>
                    ...
                    {visibleCode === t.id && (
                      <div className="border-t border-gray-700 bg-gray-900 px-4 py-4">
                        <TicketCard
                          ...
                          usedChannel={t.checked_in_channel}
                          expired={t.event_expired}
                          joinHref={!t.event_expired ? `/events/${t.event_id}/watch?code=${encodeURIComponent(t.access_token)}` : undefined}
                        />
                        <button
                          onClick={() => copyCode(t.access_token)}
                          className="w-full mt-3 bg-gray-800 hover:bg-gray-700 border border-gray-600 text-white text-xs font-medium py-2 rounded-lg transition-colors flex items-center justify-center gap-1.5"
                        >
                          {copied ? (
